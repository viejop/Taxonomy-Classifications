    ...
    while True:
        ...
        if titleInput.isdigit():
            ...
            if titleInput == 1:
                ...
                # Implement "How to use" if needed
                break
            elif titleInput == 14:
                print("Shutting down...")  # No more questions after quit
                break  # Exit the loop after quitting
            else:
                print("Input must be between 1 and 14.")
                
            while True:# Ask if the user wants to run Dogdex again
                runAgain = input("Would you like to run Dogdex again? (Y/N): ").strip().lower()
                if runAgain == 'y':
                    titleScreen()  # Recursively call titleScreen() to restart the process
                    break  # Exit the current loop to avoid recursion issues
                elif runAgain == 'n':
                    print("Goodbye!")
                    break  # Exit the loop
                else:
